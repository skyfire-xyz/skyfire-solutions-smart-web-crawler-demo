...
main() {
    ...
    case "${1:-add}" in
        ...
        "update")
            ...
            # Store current commit hash before updates
            ...
            if [ "$commit_count" -gt 1 ]; then
                print_status "Squashing $commit_count subtree update commits into single commit..."
                git reset --soft $start_commit
                git commit -m "Update all subtrees: crawler-agent-core, crawler-agent-fe, crawler-bot-protection-proxy"
            fi
            ;;
        "remove")
        ...
        "update-single")
            if [ -z "$2" ]; then
                print_error "Please specify which subtree to update: crawler-agent-core, crawler-agent-fe, or crawler-bot-protection-proxy"
                echo "Usage: $0 update-single <subtree-name>"
                exit 1
            fi
            case "$2" in
                "crawler-agent-core")
                ...
                "crawler-bot-protection-proxy")
                    ...
                    ;;
                *)
                    print_error "Invalid subtree name. Use: crawler-agent-core, crawler-agent-fe, or crawler-bot-protection-proxy"
                    exit 1
                    ;;
            esac
            ;;
        *)
            print_error "Invalid action. Use: add, update, update-single, remove, or clean"
            echo "Usage: $0 [add|update|update-single <subtree>|remove|clean]"
            exit 1
            ;;
    esac
